...
if localVersion == outdatedVersion then
    ...
    CopyDiscordURL.MouseEnter:Connect(function()
        ...
        local Siggt =  tweenService:Create(CopyDiscordURL, fadeTweenInfo2, {Size = UDim2.new(0, 352, 0, 50)})
        task.wait(0.1)
        Siggt:Play()
    end)
    ...
    CopyDiscordURL.MouseLeave:Connect(function()
        CopyDiscordURL.Text = "Copy Discord Link"
        local fadeTweenInfo2 = TweenInfo.new(0.5, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
        local Siggtt =  tweenService:Create(CopyDiscordURL, fadeTweenInfo2, {Size = UDim2.new(0, 342, 0, 40)})
        task.wait(0.1)
        Siggtt:Play()
    end)
